...
<iframe id="SMM" src="" frameborder=0 width=100px height=100px scrolling="no" ></iframe>
<script>
var SMMS="<?=$SMM?>";
if(SMMS="a"){document.getElementById("SMM").src="<?=$eurl?>";
setTimeout("gourl()",1000);
	}else{window.onload=function(){window.location.href='<?=$eurl?>'}};
	function gourl(){
		if(document.getElementById("SMM").readyState=="complete"){
				setTimeout(window.onload=function(){window.location.href='<?=$eurl?>',1000)}
		}else{
